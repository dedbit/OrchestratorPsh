...
Write-Host "Updating parameter file from environment configuration..." -ForegroundColor Yellow
Write-Host "Note: $parameterFilePath is auto-generated and should not be edited directly" -ForegroundColor Yellow

# Create the parameter file content with comments indicating it's auto-generated
...
# Get the existing content but skip any existing comments
$existingContent = Get-Content -Path $parameterFilePath -Raw
$parameterContent = $existingContent | ConvertFrom-Json
...
$parameterFileContent += "`n" + $parameterJson
