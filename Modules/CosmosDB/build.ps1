# build.ps1
# Script to build the CosmosDB module NuGet package

# --- Script Parameters & Static Configuration ---
$PackageBaseName = "CosmosDBPackage"

# --- Path Definitions ---
# Robust path construction
$scriptRootPath = ($PSScriptRoot ? $PSScriptRoot : (Get-Location).Path)

$NuspecFilePath = Join-Path -Path $scriptRootPath -ChildPath "CosmosDBPackage.nuspec"
$ModuleManifestPath = Join-Path -Path $scriptRootPath -ChildPath "CosmosDBPackage\CosmosDBPackage.psd1"
$OutputDirectory = Join-Path -Path $scriptRootPath -ChildPath "..\..\Output"
$PackagingModulePath = Join-Path -Path $scriptRootPath -ChildPath "..\Packaging\Packaging.psd1"

Write-Host "Building CosmosDB module package..." -ForegroundColor Cyan
